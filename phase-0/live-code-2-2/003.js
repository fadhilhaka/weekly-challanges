...
function numberArrayOperation(arr) {
  ...
  var hasil = []
  var hasilAkhir = 0
  //periksa tiap karakter dari arr hingga mendapatkan angkan sesuai dengan kondisi
  for (i = 0; i < arr.length; i++) {
    //bila angka genap dan bukan kelipatan empat maka kalikan bilangan tersebut dengan 3
    if (arr[i] % 2 === 0 && arr[i] % 4 !== 0) {
      var x = arr[i] * 3
      //masukkan hasil perkalian ke dalam 'hasil'
      hasil.push(x)
    //bila angka ganjil dan bukan kelipatan empat maka kalikan bilangan tersebut dengan 4
    } else if (arr[i] % 2 !== 0) {
      var y = arr[i] * 4
      //masukkan hasil perkalian ke dalam 'hasil'
      hasil.push(y)
    //bila angka kelipatan empat maka kalikan bilangan tersebut dengan 5
    } else if (arr[i] % 4 === 0) {
      var z = arr[i] * 5
      //masukkan hasil perkalian ke dalam 'hasil'
      hasil.push(z)
  ...
  //jumlahkan semua karakter dalam 'hasil'
  for (j = 0; j < hasil.length; j++) {
    hasilAkhir = hasilAkhir + hasil[j]
  }
  //tunjukkan 'hasilAkhir' yang merupakan jumlah dari semua index dalam 'hasil' 
  return hasilAkhir
}
