...
function oeprasiMatriks(arr1, arr2, operator) {
  ...
  for (var i = 0; i < arr1.length; i++) {
    ...
    for (var j = 0; j < arr1[i].length; j++) {
      //console.log(i, arr1[i].length, j, arr2[i].length)
      if (arr1[i].length !== arr2[j].length || arr1.length !== arr2.length || arr1[i].length !== arr2[i].length) {
        return 'Invalid array'
      } 
      else if (operator === '+') {
        hasil[i].push(arr1[i][j] + arr2[i][j])
      } else if (operator === '-') {
        hasil[i].push(arr1[i][j] - arr2[i][j])
      }
    }
